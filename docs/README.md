...
- 如果属于现有分类，直接放入对应目录，不要新建同义文件
- 原则上不再新增新的同级分类目录，后续新增内容只能归入现有四类中的某一类
- 只有当项目出现完全独立且长期存在的新领域时，才允许新增同级分类，并且需要先更新本文件中的分类说明
...
- 如果只是新增某一类内容，请直接在对应目录下新增文件，或优先补充已有文件
...
- 当前仓库只有页面部署工作流，不包含“发布后自动更新 Release Notes”的机制
